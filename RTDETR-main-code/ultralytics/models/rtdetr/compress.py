...
from ultralytics.nn.modules import RepConv, AIFI, RTDETRDecoder, HGStem, HGBlock, LightConv, Blocks
from ultralytics.nn.extra_modules.block import IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, \
    ...
    RepConvN, SPDConv, CSPOmniKernel, DynamicAlignFusion, DWConv, EdgeEnhancer, MutilScaleEdgeInformationSelect, Conv3XCNCSPELAN4, WTConv2d, Conv3XC, Conv3XCNCSP, Conv3XCNBottleneck, BasicBlock_PConv_Rep
from ultralytics.nn.extra_modules.attention import DualDomainSelectionMechanism
from ultralytics.nn.extra_modules.prune_module import RepNCSPELAN4_v2, Conv3XCNCSPELAN4_v2
...
def get_pruner(opt, model, example_inputs):
    ...
    #         ignored_layers.append(m)
    customized_pruners[RepConv] = RepConvPruner()
    #ignored_layers.append(model.model[4])
    #ignored_layers.append(model.model[5])
    #ignored_layers.append(model.model[6])
    #ignored_layers.append(model.model[7].branch2b[0])
    for k, m in model.named_modules():
        if isinstance(m, AIFI):
            ignored_layers.append(m)
        if isinstance(m, RTDETRDecoder):
            ignored_layers.append(m)
        if isinstance(m, Conv3XCNCSPELAN4_v2):
    #        ignored_layers.append(m.cv0)
    #        ignored_layers.append(m.cv1)
            ignored_layers.append(m.cv2[0].m[0])
            ignored_layers.append(m.cv3[0].m[0])
            ignored_layers.append(m.cv4)
        if isinstance(m, WTConv2d):
            ignored_layers.append(m.base_conv)
            ignored_layers.append(m.wavelet_convs[0])
        if isinstance(m, BasicBlock_PConv_Rep):
            ignored_layers.append(m.branch2b[0].partial_conv3)
