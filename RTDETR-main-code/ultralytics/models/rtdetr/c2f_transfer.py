...
from ultralytics.nn.extra_modules.block import C2f_FFCM, RGCSPELAN, RepNCSPELAN4, CSP_MutilScaleEdgeInformationSelect, Conv3XCNCSPELAN4
...
def replace_c2f_with_c2f_v2(module):
    ...
    #         replace_c2f_with_c2f_v2(child_module)
    for name, child_module in module.named_children():
        if isinstance(child_module, Conv3XCNCSPELAN4):
            # Replace C2f with C2f_v2 while preserving its parameters
            c2f_v2 = Conv3XCNCSPELAN4_v2(child_module.cv1.conv.in_channels, child_module.cv4.conv.out_channels,
                            child_module.cv1.conv.out_channels, child_module.cv3[-1].conv.out_channels, 1)
            transfer_weights_elan_to_elan_v2(child_module, c2f_v2)
            setattr(module, name, c2f_v2)
        else:
